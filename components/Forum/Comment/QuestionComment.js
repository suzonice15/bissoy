...
function Comment({
  ...
  async function editComment() {
    ...
    setEditing(false);
    try {
      const res = await axios.post(`qc/${comment_id}/edit`, {
...
export default function QuestionComment({ question_id, askToLogin }) {
  ...
  useEffect(() => {
    async function fetchComments() {
      setDataLoading(true);
      const res = await axios.get(`qc/${question_id}`);
      if (res.status === 200) {
        setData(res.data);
      }
  ...
  async function addComment() {
    ...
    setData([new_comment, ...data]);
    try {
      setLoading(true);
      const res = await axios.post(`add_qc/${question_id}`, {
        comment_body: comment,
      });
      if (res.status !== 200) {
  ...
  async function handleDelete(cm_id) {
    ...
    try {
      const res = await axios.post(`qc/${cm_id}/delete`);
      if (res.status !== 200) {
        Toast.show({
